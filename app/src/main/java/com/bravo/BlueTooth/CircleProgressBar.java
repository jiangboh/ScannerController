...
//        float result = (currentValue * 100.0f / maxValue * 1.0f); // 计算进度
        String percent = getResources().getString(R.string.tt_signal);//String.format("%.1f", result) + "%";
        textPaint.setTextSize(150);
        if (currentValue != 0) {
            percent = String.valueOf(currentValue);
            textPaint.setTextSize(500); // 设置要绘制的文字大小
        }
