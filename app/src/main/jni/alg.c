...
//jboolean OP[16] = {0x63, 0xbf, 0xa5, 0x0e, 0xe6, 0x52, 0x33, 0x65,
//       0xff, 0x14, 0xc1, 0xf4, 0x5f, 0x88, 0x73, 0x7d};
#if 0
jboolean OP[16] = {0xcd, 0xc2, 0x02, 0xd5, 0x12, 0x3e, 0x20, 0xf6,
...
#if 0
void f1(jboolean k[16], jboolean rand[16], jboolean sqn[6], jboolean amf[2],
        jboolean mac_a[8]);
void f2345(jboolean k[16], jboolean rand[16],
           jboolean res[8], jboolean ck[16], jboolean ik[16], jboolean ak[6]);
void f1star(jboolean k[16], jboolean rand[16], jboolean sqn[6], jboolean amf[2],
            jboolean mac_s[8]);
void f5star(jboolean k[16], jboolean rand[16],
            jboolean ak[6]);
void ComputeOPc(jboolean op_c[16]);
void RijndaelKeySchedule(jboolean key[16]);
void RijndaelEncrypt(jboolean input[16], jboolean output[16]);
#endif
...
void f2345(jboolean k[16], jboolean rand[16], jboolean res[8], jboolean ck[16], jboolean ik[16], jboolean ak[6])
{
    /*jboolean op_c[16] = {0x12,0x34,0x56,0x78,0x90,0x12,0x34,0x56,0x78,
                    0x90,0x12,0x34,0x56,0x78,0x90,0x12};*/
    jboolean op_c[16];
    jboolean temp[16];
    jboolean out[16];
    jboolean rijndaelInput[16];
...
void f1star(jboolean k[16], jboolean rand[16], jboolean sqn[6], jboolean amf[2], jboolean mac_s[8])
{
    jboolean op_c[16];
    jboolean temp[16];
    jboolean in1[16];
...
void f5star(jboolean k[16], jboolean rand[16], jboolean ak[6])
{
    jboolean op_c[16];
    jboolean temp[16];
    jboolean out[16];
...
#if 0
void f8(jboolean *key, unsigned long count, unsigned long bearer, unsigned long dir, jboolean *data, unsigned long length)
{
    unsigned long K[4], IV[4];
    int n = (length + 31) / 32;
    int i = 0;
    unsigned long *KS;
    ...
    Initialize(K, IV);
    KS = (unsigned long *)malloc(4 * n);
    GenerateKeystream(n, (unsigned long*)KS);
    for(i = 0; i < n; i++)
    {
        data[4 * i + 0] ^= (jboolean)(KS[i] >> 24)
                           data[4 * i + 1] ^= (jboolean)(KS[i] >> 16)
                                              data[4 * i + 2] ^= (jboolean)(KS[i] >> 8)
                                                      data[4 * i + 3] ^= (jboolean)(KS[i])
    }
    free(KS);
}
#endif

jboolean mask8bit_EIA1(int n)
{
    return 0xFF ^ ((1 << (8 - n)) - 1);
}
...
void ComputeOPc( jboolean op_c[16] )
{
	//lmj 2016-11-16
	int i;
    jboolean cOP[16] = {0x12,0x34,0x56,0x78,0x90,0x12,0x34,0x56,0x78,0x90,0x12,0x34,0x56,0x78,0x90,0x12};
    
	RijndaelEncrypt(cOP, op_c );
	for (i=0; i<16; i++)
		op_c[i] ^= cOP[i];
...
void KeyAdd(jboolean state[4][4], jboolean roundKeys[11][4][4], int round)
{
    int i, j;
...
int ByteSub(jboolean state[4][4])
{
    int i, j;
...
void ShiftRow(jboolean state[4][4])
{
    jboolean temp;
...
void MixColumn(jboolean state[4][4])
{
    jboolean temp, tmp, tmp0;
    int i;
...
void c2(jboolean xres[8], jboolean sres[4])
{
    int i;
...
void c3(jboolean ck[16], jboolean ik[16], jboolean kc[8])
{
    int i;
...
void fautn(jboolean sqn[6], jboolean ak[6], jboolean amf[2], jboolean mac_a[8], jboolean autn[16])
{
    int i;
...
void fautn1(jboolean sqn[6], jboolean ak[6], jboolean amf[2], jboolean mac_a[8], jboolean autn[16])
{
    int i;
...
void fauts1(jboolean sqn[6], jboolean ak[6], jboolean mac_s[8], jboolean auts[16])
{
    int i;
    for(i = 0; i < 6; i++)
    {
...
void fauts(jboolean auts[14], jboolean sqn[6], jboolean ak[6], jboolean mac_s[8])
{
    int i;
...
/* Extract a byte from a 32-bit unsigned long */

#define byte0(x) ((jboolean)(x))
#define byte1(x) ((jboolean)(x >> 8))
#define byte2(x) ((jboolean)(x >> 16))
#define byte3(x) ((jboolean)(x >> 24))

/* Put or get a 32 bit unsigned long (v) in machine order from a byte *
 * address in (x) */

#ifdef LITTLE_ENDIAN
#define u32_in(x) (*(unsigned long*)(x))
#define u32_out(x,y) (*(unsigned long*)(x) = y)
#else
/* Invert byte order in a 32 bit variable */
__inline unsigned long byte_swap(const unsigned long x)
{
    return rot1(x) & 0x00ff00ff | rot3(x) & 0xff00ff00;
}
__inline unsigned long u32_in(const jboolean x[])
{
    return byte_swap(*(unsigned long*)x);
};
__inline void u32_out(jboolean x[], const unsigned long v)
{
    *(unsigned long*)x = byte_swap(v);
};
#endif

/*--------------- The lookup tables ----------------------------*/
static unsigned long rnd_con[10] =
{
...
/*----------------- The workspace ------------------------------*/
static unsigned long Ekey[44]; /* The expanded key */
...
void Disp(const char *str, jboolean *val, int len)
{
...
int Test()
{
//jboolean k[16]={0xb1,0x57,0x7f,0x45,0xce,0x0b,0xf9,0xda,0x87,0xb2,0x59,0xaa,0xd6,0x0d,0xc2,0xdd};
//jboolean k[16]={0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x09,0x0a,0x0b,0x0c,0x0d,0x0e,0x0f};
    jboolean k[16] = {0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa};
//jboolean rand[16]={0x23,0x55,0x3c,0xbe,0x96,0x37,0xa8,0x9d,0x21,0x8a,0xe6,0x4d,0xae,0x47,0xbf,0x35};
//jboolean rand[16]={0x23,0x55,0x3c,0xbe,0x96,0x37,0xa8,0x9d,0x21,0x8a,0xe6,0x4d,0xae,0x47,0xbf,0x35};
//jboolean rand[16]={0x00,0x00,0x4c,0xb3,0x00,0x00,0x2c,0x02,0x00,0x00,0x0e,0x44,0x00,0x00,0x01,0xb9};
    jboolean rand[16] = {0x00, 0x00, 0x6a, 0xbf, 0x00, 0x00, 0x1e, 0x42, 0x00, 0x00, 0x59, 0xef, 0x00, 0x00, 0x25, 0xb7};
//jboolean rand[16] = {0x00,0x00,0x61,0x11,0x00,0x00,0x6c,0xb5,0x00,0x00,0x6e,0xde,0x00,0x00,0x5f,0x07};
//jboolean rand[16]={0x72,0x9b,0x02,0xcc,0xd9,0xaa,0xde,0xe5,0xf0,0xe4,0xd4,0x6c,0xa2,0x31,0x72,0xdc};
    jboolean sqn[6] = {0x00, 0x11, 0x11, 0x11, 0x11, 0x11};
//jboolean sqn[6]={0x00,0x00,0x20,0x00,0x00,0x00};
    jboolean amf[2] = {0x00, 0x00};
    jboolean res[8];
    jboolean ck[16];
    jboolean ik[16];
    jboolean ak1[6];
    ...
    jboolean autn[16];
    jboolean auts[14];
    jboolean opc[16] = {0xD9, 0x16, 0x1E, 0x34, 0x5C, 0x65, 0x95, 0x6E, 0xA0,
                             0x6F, 0x6B, 0x83, 0x38, 0x10, 0x31, 0xAC
                            };

    jboolean auts1[14] = {0x15, 0x21, 0xab, 0xb9, 0x0c, 0xae, 0x08, 0x4f, 0xa9, 0x5c, 0xd7, 0xa0, 0x35, 0x67};
    jboolean autn1[16] = {0x7e, 0x45, 0x46, 0x76, 0xa1, 0x8b, 0x00, 0x00, 0x7a, 0x8c, 0x2d, 0xd2, 0x7f, 0xdb, 0x39, 0xad};
    ...
    fautn(sqn, ak, amf, mac_a, autn);

//jboolean autn1[16]={0x7e,0x45,0x26,0x76,0xa1,0x8a,0x00,0x00,0x7a,0x8c,0x2d,0xd2,0x7f,0xdb,0x39,0xad};

//jboolean autn1[16]={0x7e,0x45,0x86,0x76,0xa1,0x8d,0x00,0x00,0x7a,0x8c,0x2d,0xd2,0x7f,0xdb,0x39,0xad};
    fautn1(sqn, ak, amf, mac_a, autn1);
    Disp("sqn", sqn, 6);

    f1star(k, rand, sqn, amf, mac_s);
    f5star(k, rand, ak1);
    fauts1(sqn, ak1, mac_s, auts);

    fauts(auts1, sqn, ak1, mac_s);
    Disp("sqn", sqn, 6);
    Disp("mac_s", mac_s, 8);
